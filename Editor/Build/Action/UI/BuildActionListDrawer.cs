...
namespace SuperSystems.UnityBuild
{
    ...
    public class BuildActionListDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            actionTypes = FindActionTypesByProperty(property);
        ...
        private void DrawActionList(SerializedProperty property, List<Type> actionTypes)
        {
            if (actionTypes.Count > 0)
            {
                ...
                index = EditorGUILayout.Popup(index, buildActionNameList, UnityBuildGUIUtility.popupStyle,
                    GUILayout.ExpandWidth(true));
                if (GUILayout.Button("Add Build Action", GUILayout.ExpandWidth(false), GUILayout.MaxWidth(150)) &&
                    index < actionTypes.Count)
                {
        ...
        BuildAction[] FindActionsByProperty(SerializedProperty property)
        {
            BuildAction[] buildActions;
            ...
            else if (property.name.ToUpper().Contains("PROCESS"))
            {
                buildActions = BuildSettings.postProcessBuildActions.buildActions;
            }
            else
            {
        ...
        List<Type> FindActionTypesByProperty(SerializedProperty property)
        {
            List<Type> actionTypes;
            if (property.name.ToUpper().Contains("PRE"))
            {
            ...
            {
                actionTypes = BuildActionListUtility.postProcessBuildActions;
            }
            else
            {
