...
namespace SuperSystems.UnityBuild
{
    public static class BuildProject
    {
        ...
        public static string GenerateBuildPath(string prototype, BuildReleaseType releaseType,
            BuildPlatform buildPlatform, BuildArchitecture arch, BuildDistribution dist, DateTime buildTime)
        {
        ...
        private static void ReplaceFromFile(StringBuilder sb, string keyString, string filename)
        {
            if (sb.ToString().IndexOf(keyString) > -1)
            {
                string[] fileSearchResults =
                    Directory.GetFiles(Application.dataPath, filename, SearchOption.AllDirectories);
                string filePath = null;
                string desiredFilePath =
                    string.Format("UnityBuild{0}Editor{0}{1}", Path.DirectorySeparatorChar, filename);
        ...
        private static void PerformBuild(string[] buildConfigs, BuildOptions options = BuildOptions.None)
        {
            ...
            for (int i = 0; i < buildConfigs.Length; i++)
            {
                ...
                BuildSettings.projectConfigurations.ParseKeychain(configKey, out releaseType, out platform, out arch,
                    out dist);
                bool success = BuildPlayer(notification, releaseType, platform, arch, dist, buildTime, options,
                    configKey);
        ...
        private static bool BuildPlayer(string notification, BuildReleaseType releaseType, BuildPlatform platform,
            ...
            string configKey)
        {
            ...
            // Generate build path.
            string buildPath = GenerateBuildPath(BuildSettings.basicSettings.buildPath, releaseType, platform,
                architecture, distribution, buildTime);
            ...
            // Pre-build actions.
            PerformPreBuild(releaseType, platform, architecture, distribution, buildTime, ref options, configKey,
                buildPath);

            // Generate BuildConstants.
            BuildConstantsGenerator.Generate(buildTime, BuildSettings.productParameters.lastGeneratedVersion,
            ...
            UnityEditor.Build.Reporting.BuildReport buildReport = BuildPipeline.BuildPlayer(
                releaseType.sceneList.GetSceneFileList(), Path.Combine(buildPath, exeName), architecture.target,
                options);
...
#else
        error =
 BuildPipeline.BuildPlayer(releaseType.sceneList.GetSceneFileList(), Path.Combine(buildPath, exeName), architecture.target, options);
#endif
            ...
            // Post-build actions.
            PerformPostBuild(releaseType, platform, architecture, distribution, buildTime, ref options, configKey,
                buildPath);
